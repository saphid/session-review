...
function applyEvent(snapshot, entry, inheritedDiagnostics = []) {
  ...
  switch (event.type) {
    ...
    case 'complete':
      ...
      next.pendingEventSeq = null;
      next.pendingEvent = null;
      // Completion is only valid after manual carbonize cleanup has been
      // verified. Drop the stale required-cleanup diagnostic so status/resume
      // output does not make a completed session look stuck.
      next.diagnostics = next.diagnostics.filter((d) => d.error !== 'carbonize_cleanup_required');
      break;
    case 'agent_error':
