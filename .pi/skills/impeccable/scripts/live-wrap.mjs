...
The agent should insert variant HTML at insertLine.`);
  ...
  const inline = Number.isInteger(match.startColumn) && Number.isInteger(match.endColumn);
  const sourceLine = lines[startLine];
  const inlineBefore = inline ? sourceLine.slice(0, match.startColumn) : '';
  const inlineAfter = inline ? sourceLine.slice(match.endColumn) : '';
  const inlineOuterIndent = sourceLine.match(/^(\s*)/)[1];
  const inlineNestedIndent = inlineOuterIndent + (inlineBefore.trim() ? '  ' : '');
  const indent = inline ? inlineNestedIndent : lines[startLine].match(/^(\s*)/)[1];
  ...
  const originalLines = inline ? [sourceLine.slice(match.startColumn, match.endColumn)] : lines.slice(startLine, endLine + 1);
  ...
  // Replace the original element with the wrapper. For compact same-line HTML
  // (`<div><button id="x">…</button><button>…</button></div>`), split the line
  // around the selected element so live mode wraps the picked child, not the
  // outer line or sibling controls.
  const inlineBeforeLines = inline && inlineBefore.trim() ? [inlineBefore.trimEnd()] : [];
  const inlineAfterLines = inline && inlineAfter.trim() ? [inlineNestedIndent + inlineAfter.trimStart()] : [];
  const wrapperStartLine = startLine + inlineBeforeLines.length;
  const newLines = [
  ...
  // extra line count.
  const insertLine = wrapperStartLine + 6 + (originalLines.length - 1);

  console.log(JSON.stringify({
    file: path.relative(process.cwd(), targetFile),
    startLine: wrapperStartLine + 1,       // 1-indexed for the agent
    ...
    endLine: wrapperStartLine + wrapperLines.length + (originalLines.length - 1), // 1-indexed
...
function findElement(lines, query, tag = null) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const queryColumn = lines[i].indexOf(query);
    if (queryColumn === -1) continue;
    ...
    const inlineSpan = findInlineElementSpan(lines[i], queryColumn, tag);
    if (inlineSpan) return { startLine: i, endLine: i, ...inlineSpan };

    const openerLine = findOpenerLine(lines, i, tag, queryColumn);
...
function findAllElements(lines, query, tag = null) {
  ...
  const seen = new Set();
  for (let i = 0; i < lines.length; i++) {
    let queryColumn = lines[i].indexOf(query);
    if (queryColumn === -1) continue;
    const stripped = lines[i].trim();
    if (stripped.startsWith('<!--') || stripped.startsWith('{/*') || stripped.startsWith('//')) continue;
    if (lines[i].includes('data-impeccable-variant')) continue;
    while (queryColumn !== -1) {
      const inlineSpan = findInlineElementSpan(lines[i], queryColumn, tag);
      if (inlineSpan) {
        const key = `${i}:${inlineSpan.startColumn}:${inlineSpan.endColumn}`;
        if (!seen.has(key)) {
          seen.add(key);
          out.push({ startLine: i, endLine: i, ...inlineSpan });
        }
        queryColumn = lines[i].indexOf(query, queryColumn + query.length);
        continue;
      }

      const openerLine = findOpenerLine(lines, i, tag, queryColumn);
      if (openerLine !== -1 && !seen.has(openerLine)) {
        seen.add(openerLine);
        const endLine = findClosingLine(lines, openerLine);
 ...
 * one or more tags before the matched query, return the closest matching tag
 * opener on that line. This matters for compact HTML like
 * `<div><button id="x">…</button></div>` where the query is on the same line
 * as an outer wrapper and the selected element is the later nested opener.
 * Otherwise walk up to 10 lines backward looking for the first tag opener. If
 * `tag` is specified, the opener must match that tag name; an opener with a
 * different tag name aborts the backward walk for this match (we don't jump
 * across element boundaries).
 *
 * Returns the line index of the opener, or -1 if none can be resolved.
 */
function findOpenerLine(lines, matchLine, tag, matchColumn = null) {
  const sameLine = findSameLineOpener(lines[matchLine], tag, matchColumn);
  if (sameLine) return matchLine;

  const MAX_BACKWALK = 10;
  for (let i = matchLine - 1; i >= Math.max(0, matchLine - MAX_BACKWALK); i--) {
    const opener = findSameLineOpener(lines[i], tag, null);
    if (!opener) continue;
    if (!tag || opener.tag === tag) return i;
...
function findSameLineOpener(line, tag, beforeColumn = null) {
  const re = new RegExp(OPENER_RE.source, 'g');
  let match;
  let closest = null;
  const limit = beforeColumn ?? line.length;
  while ((match = re.exec(line)) !== null) {
    if (match.index > limit) break;
    if (tag && match[1] !== tag) continue;
    closest = { tag: match[1], index: match.index };
  }
  return closest;
}

function findInlineElementSpan(line, queryColumn, tag) {
  const opener = findSameLineOpener(line, tag, queryColumn);
  if (!opener) return null;

  const openEnd = line.indexOf('>', opener.index);
  if (openEnd === -1 || queryColumn > openEnd) return null;

  const openText = line.slice(opener.index, openEnd + 1);
  if (/\/\s*>$/.test(openText)) return { startColumn: opener.index, endColumn: openEnd + 1 };

  const closeStart = line.indexOf(`</${opener.tag}`, openEnd + 1);
  if (closeStart === -1) return null;
  const closeEnd = line.indexOf('>', closeStart);
  if (closeEnd === -1) return null;

  return { startColumn: opener.index, endColumn: closeEnd + 1 };
}
